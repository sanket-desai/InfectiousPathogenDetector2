...
class IPDLongRead(object):
    ...
    def variantcalling(self):
        ...
        cprocess=subprocess.run(cmd, shell=True, stdout=subprocess.DEVNULL)
        cprocess.check_returncode()
        cmd=GlobalVar.varscan_+" mpileup2cns "+self.primarysortedbam_.replace(".bam",".mpileup")+" --min-coverage 4 --output-vcf 1 --variants > "+self.primarysortedbam_.replace(".bam","_varscan.vcf")
        cprocess=subprocess.run(cmd, shell=True, stdout=subprocess.DEVNULL)
        cprocess.check_returncode()
        cmd=GlobalVar.lofreq_+" call -f "+GlobalVar.pathofa_+" -o "+self.primarysortedbam_.replace(".bam","_lofreq1.vcf")+" "+self.primarysortedbam_
        cprocess=subprocess.run(cmd, shell=True, stdout=subprocess.DEVNULL)
        cprocess.check_returncode()
        cmd=GlobalVar.vcffilter_+" -f \"DP > 3 & AF > 0.5\" "+ self.primarysortedbam_.replace(".bam","_lofreq1.vcf") +" > "+self.primarysortedbam_.replace(".bam","_lofreq.vcf")
        #print(cmd)
        ...
        cprocess=subprocess.run(cmd, shell=True, stdout=subprocess.DEVNULL)
        cprocess.check_returncode()
        cmd=GlobalVar.freebayes_+" -f "+GlobalVar.pathofa_+"  --min-repeat-entropy 1 --haplotype-length 500 --haplotype-basis-alleles "+ self.primarysortedbam_.replace(".bam","_freebayes_r1.vcf.gz")+" "+self.primarysortedbam_+" | "+ GlobalVar.vcffilter_+" -f \"QUAL > 20 & DP > 3 & AF > 0.5\" > "  +self.primarysortedbam_.replace(".bam","_freebayes.vcf")
        cprocess=subprocess.run(cmd, shell=True, stdout=subprocess.DEVNULL)
        cprocess.check_returncode()
